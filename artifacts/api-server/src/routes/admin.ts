...
router.get("/stats", requireAdmin, async (req, res) => {
  try {
    ...
    const [proUsers] = await db
      .select({ count: count() })
      .from(usersTable)
      .where(eq(usersTable.currentPlan, "pro"));
    const [businessUsers] = await db
      .select({ count: count() })
      .from(usersTable)
      .where(eq(usersTable.currentPlan, "business"));
    ...
    const [emailsToday] = await db
      .select({ count: count() })
      .from(emailsTable)
      .where(gte(emailsTable.receivedAt, today));
    ...
    const result = await Promise.all(
      users.map(async (u) => {
        const [inboxCount] = await db
          .select({ count: count() })
          .from(inboxesTable)
          .where(eq(inboxesTable.userId, u.id));
...
router.patch("/users/:userId", requireAdmin, async (req, res) => {
  try {
    ...
    if (plan) updates.currentPlan = plan as any;
    if (grantCredits)
      updates.credits = Math.min(user.credits + grantCredits, user.maxCredits);
...
router.get("/inboxes", requireAdmin, async (req, res) => {
  try {
    ...
    const result = await Promise.all(
      inboxes.map(async ({ inbox, user: u }) => {
        const [emailCount] = await db
          .select({ count: count() })
          .from(emailsTable)
...
router.get("/revenue", requireAdmin, async (req, res) => {
  try {
    ...
    const [freeCount] = await db
      .select({ count: count() })
      .from(usersTable)
      .where(eq(usersTable.currentPlan, "free"));
    ...
    const [bizCount] = await db
      ...
      .where(eq(usersTable.currentPlan, "business"));
    const total =
      Number(freeCount?.count || 0) +
      Number(proCount?.count || 0) +
      Number(bizCount?.count || 0);

    const planDistribution = [
      {
        plan: "free",
        count: Number(freeCount?.count || 0),
        percentage: total ? (Number(freeCount?.count || 0) / total) * 100 : 0,
      },
      {
        plan: "pro",
        count: Number(proCount?.count || 0),
        percentage: total ? (Number(proCount?.count || 0) / total) * 100 : 0,
      },
      {
        plan: "business",
        count: Number(bizCount?.count || 0),
        percentage: total ? (Number(bizCount?.count || 0) / total) * 100 : 0,
      },
    ];
