...
router.get("/current", requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      nextPlanName: user.nextPlan ? (planNames[user.nextPlan] || user.nextPlan) : null,
...
router.post("/schedule-downgrade", requireAuth, async (req, res) => {
  try {
    ...
    if (comparePlans(planId, user.currentPlan) >= 0) {
      res.status(400).json({ error: "You can only schedule a downgrade to a lower plan" }); return;
    }
    if (user.currentPlan === "free" || !user.planExpiryDate) {
      res.status(400).json({ error: "No active paid plan to downgrade from" }); return;
    }
