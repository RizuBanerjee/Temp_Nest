...
router.get("/:emailId", requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      id: email.id, inboxId: email.inboxId, from: email.fromAddress,
      subject: email.subject, bodyText: email.bodyText, bodyHtml: email.bodyHtml,
      isRead: email.isRead, hasOtp: email.hasOtp, otpCode: email.otpCode,
...
router.patch("/:emailId/read", requireAuth, async (req, res) => {
  try {
    ...
    const [email] = await db.update(emailsTable)
    ...
    res.json({
      id: email.id, inboxId: email.inboxId, from: email.fromAddress, subject: email.subject,
      preview: email.preview, isRead: email.isRead, hasOtp: email.hasOtp, otpCode: email.otpCode,
      createdAt: email.receivedAt.toISOString(),
    });
  } catch (err) {
...
router.get("/:emailId/otp", requireAuth, async (req, res) => {
  try {
    ...
    if (email.hasOtp && email.otpCode) {
      res.json({ found: true, code: email.otpCode, length: email.otpCode.length }); return;
    }
    ...
    if (code) {
      await db.update(emailsTable).set({ hasOtp: true, otpCode: code }).where(eq(emailsTable.id, emailId));
      const existing = await db.select().from(otpRecordsTable).where(eq(otpRecordsTable.emailId, emailId)).limit(1);
      if (!existing[0]) {
        await db.insert(otpRecordsTable).values({ emailId, userId: user.id, code, length: code.length });
      }
    }

    res.json({ found: !!code, code: code ?? null, length: code ? code.length : null });
  } catch (err) {
