...
export default function InboxDetail({ params }: { params: { id: string } }) {
  ...
  async function doRefresh(silent = false) {
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: getListInboxEmailsQueryKey(id),
      });
      ...
      if (!silent)
        toast.success(
          `${result.newEmails} new email${result.newEmails !== 1 ? "s" : ""}`,
        );
      else if (result.newEmails > 0)
        toast.success(
          `${result.newEmails} new email${result.newEmails !== 1 ? "s" : ""} arrived`,
        );
    } catch (err: any) {
      if (!silent)
  ...
  async function doPoll() {
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: getGetInboxQueryKey(id),
      });
      setLastRefresh(new Date());
    } catch {}
  }
              ...
              {inboxLoading ? (
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {inbox?.isPriority && (
                      <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/20 text-xs">
                        Priority
                      </Badge>
                    )}
                  </div>
          ...
          {emailsLoading ? (
            ...
            <div className="text-center py-20 border border-dashed border-border rounded-2xl">
              ...
              <p className="text-sm text-muted-foreground mb-2">
                Share this address and emails will appear here automatically.
              </p>
              <p className="text-xs text-muted-foreground font-mono bg-muted/40 px-3 py-1.5 rounded-lg inline-block">
                {inbox?.address}
              </p>
              <p className="text-xs text-muted-foreground mt-4">
                Auto-refreshing every 30 seconds · {countdown}s until next check
              </p>
            </div>
          ) : (
            <div className="space-y-2">
                  ...
                  >
                    <div className="mt-1.5">
                      {email.hasOtp ? (
                        ...
                        <div
                          className={`w-2 h-2 rounded-full ${email.isRead ? "bg-transparent border border-border" : "bg-primary"}`}
                        />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
